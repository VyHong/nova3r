...
class SSI3DScore(nn.Module):
    ...
    def chamfer_and_fscore(self, pred, gt, eval_layers, use_single_dir=False):
        ...
        details = {}
        details = {
            "CD_{}_{}".format(self.num_eval_pts, eval_layers if eval_layers else "full"): (
                float(chamfer_dist.mean()),
                int(chamfer_dist.shape[0]),
            )
        }
        ...
        if not isinstance(self.fs_thres, list):
            ...
            details.update(
                {
                    "f_score_{}_{}".format(self.fs_thres, eval_layers if eval_layers else "full"): (
                        float(f_score.mean()),
                        int(f_score.shape[0]),
                    )
                }
            ...
            for thres in self.fs_thres:
                ...
                details.update(
                    {
                        "f_score_{}_{}".format(thres, eval_layers if eval_layers else "full"): (
                            float(f_score.mean()),
                            int(f_score.shape[0]),
                        )
                    }
    ...
    def forward(self, data, pred, **kw):
        ...
        for eval_layers in [None, "visible", "visible_single"]:
            ...
            if pts3d_pred.shape[1] > pts3d_uniform_gt.shape[1]:
                pts3d_pred_eval, _ = sampling_train_gen_target(
                    pts3d_pred, pts3d_pred_mask, None, target_sampling="random", batch_size=self.num_eval_pts
                )
            else:
            ...
            if self.use_cd_align:
                if align_shift is not None and align_scale is not None:
                    ...
                    pts3d_pred_eval, shift, scale = scale_shift_alignment_chamfer(
                        pts3d_pred_eval, pts3d_uniform_gt, max_iterations=400, lr=0.03, return_transform=True
                    )
                    align_shift = shift
                    align_scale = scale
            ...
            if valid_batch_mask is not None:
                details = self.chamfer_and_fscore(
                    pts3d_pred_eval[valid_batch_mask],
                    pts3d_uniform_gt[valid_batch_mask],
