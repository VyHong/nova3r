...
class FMBlock(nn.Module):
    def __init__(self, d_model, d_point, d_time, d_shape, num_heads=8, mlp_ratio=4, dropout=0.0, activation="gelu"):
        ...
        self.adaLN_modulation = nn.Sequential(nn.Linear(d_time, d_model, bias=True), nn.SiLU(), nn.Linear(d_model, 6 * d_model, bias=True))
...
class Attention(nn.Module):
    def __init__(self, query_dim, context_dim=None, num_heads=8, qkv_bias=False, use_sdpa=False):
        super().__init__()
...
class CrossAttnBlock(nn.Module):
    def __init__(self, hidden_size, context_dim, time_dim=None, num_heads=1, mlp_ratio=4.0, **block_kwargs):
        super().__init__()
        self.norm1 = nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-6)
        self.norm_context = nn.LayerNorm(hidden_size)
        self.cross_attn = Attention(hidden_size, context_dim=context_dim, num_heads=num_heads, qkv_bias=True, **block_kwargs)
        ...
        if time_dim is not None:
            self.adaLN_modulation = nn.Sequential(
                nn.Linear(time_dim, hidden_size, bias=True), nn.SiLU(), nn.Linear(hidden_size, 6 * hidden_size, bias=True)
            )
    ...
    def forward(self, x, context, pos_encoded_time=None, mask=None):
        ...
        x = x + gate_msa * self.cross_attn(x1, context=self.norm_context(context), attn_bias=attn_bias)
...
class CrossAttnContextBlock(nn.Module):
    def __init__(self, hidden_size, context_dim, time_dim=None, num_heads=1, mlp_ratio=4.0, **block_kwargs):
        ...
        if time_dim is not None:
            self.adaLN_modulation = nn.Sequential(
                nn.Linear(time_dim, hidden_size, bias=True), nn.SiLU(), nn.Linear(hidden_size, 2 * hidden_size, bias=True)
            )
        self.time_dim = time_dim
    ...
    def forward(self, x, context, pos_encoded_time=None, mask=None):
        ...
        x = x + self.cross_attn(x1, context=context_mod, attn_bias=attn_bias)
        # x2 = modulate(self.norm2(x), shift_mlp, scale_mlp)
        x2 = self.norm2(x)
        x = x + self.mlp(x2)
        return x
...
class AttnBlock(nn.Module):
    def __init__(self, hidden_size, num_heads, attn_class=Attention, mlp_ratio=4.0, **block_kwargs):
        super().__init__()
        self.norm1 = nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-6)
    ...
    def forward(self, x, mask=None):
        attn_bias = mask
        if mask is not None:
            mask = (mask[:, None] * mask[:, :, None]).unsqueeze(1).expand(-1, self.attn.num_heads, -1, -1)
            max_neg_value = -torch.finfo(x.dtype).max
            attn_bias = (~mask) * max_neg_value
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.self_point2virtual_blocks = nn.ModuleList(
            [
                CrossAttnBlock(d_model, d_model, time_dim=d_time, num_heads=num_heads, mlp_ratio=mlp_ratio, use_sdpa=self.use_sdpa)
        ...
        self.self_virtual2point_blocks = nn.ModuleList(
            [
                CrossAttnContextBlock(d_model, d_model, num_heads=num_heads, time_dim=d_time, mlp_ratio=mlp_ratio, use_sdpa=self.use_sdpa)
                for _ in range(self_depth)
            ]
        )

        self.virual_tracks = nn.Parameter(torch.randn(1, num_virtual_tracks, d_model))
...
class LatentJointFMDecoder(nn.Module):
    """Flow matching decoder that predicts velocities for noised latent tokens.

    This mirrors :class:`PointJointFMDecoderV2`, but uses the current noised
    latent state as the query sequence instead of Fourier-embedded 3D points.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.use_num_view_cond:
            self.view_embedder = TimestepEmbedder(d_model)
    ...
    def forward(self, decout, noised_latents=None, query_points=None, timestep=None, num_views=None, **kwargs):
        """
        noised_latents: [B, N, C]
        timestep: [B] or [B, N]
        """
        if noised_latents is None:
            noised_latents = query_points
        if noised_latents is None:
            raise ValueError("LatentJointFMDecoderV2 requires noised_latents.")
        if timestep is None:
            raise ValueError("LatentJointFMDecoderV2 requires timestep.")
        ...
        if timestep.dim() == 0:
            timestep = timestep.reshape(1).expand(B)
        if timestep.dim() == 1:
            timestep = repeat(timestep, "b -> b n", n=noised_latents.shape[1])

        timestep = rearrange(timestep, "b n -> (b n)")
        pos_encoded_time = self.t_embedder(timestep)
        pos_encoded_time = rearrange(pos_encoded_time, "(b n) d -> b n d", b=B)

        if self.use_num_view_cond and num_views is not None:
            num_views = repeat(num_views, "b -> (b n)", n=noised_latents.shape[1])
            num_views = num_views / 8.0
            pos_encoded_view = self.view_embedder(num_views)
            pos_encoded_view = rearrange(pos_encoded_view, "(b n) d -> b n d", b=B)
            pos_encoded_time = pos_encoded_time + pos_encoded_view

        x = self.latent_embed(noised_latents)
