...
if __name__ == "__main__":
    ...
    # Create a dummy input tensor with shape (batch_size, num_points, in_channels)
    input_tensor = torch.randn(2, 2048, 6)
    # Encode the input tensor
    encoded_output = model.encode(input_tensor, num_tokens=2048)

    print("Encoded output shape:", encoded_output.latent_dist.shape)
    # Decode the encoded output
    sampled_points = torch.randn(2, 20480, 3)  # Dummy
    decoded_output = model.decode(encoded_output.latent_dist, sampled_points)

    print("Decoded output shape:", decoded_output.sample.shape)