...
sys.modules["utils"] = __import__("training.nova3r.data.datasets.replica_utils")
sys.modules["utils.igibson_utils"] = __import__("training.nova3r.data.datasets.replica_utils.igibson_utils")

igibson_colorbox_temp = np.array(sns.hls_palette(n_colors=60, l=0.45, s=0.8))
igibson_colorbox = []
...
def layout_line_segment_indexes(N):
    layout_lines = [[i, (i + 1) % N] for i in range(N)] + [[i + N, (i + 1) % N + N] for i in range(N)] + [[i, i + N] for i in range(N)]
    return layout_lines
...
sys.modules.setdefault("training", __import__("training"))
sys.modules["training"].Camera = Camera


def detectron_gt_sample(data, idx=None):
    record = {"file_name": data["image_path"]["rgb"], "image_id": idx, "height": data["camera"]["height"], "width": data["camera"]["width"]}
    annotations = []
