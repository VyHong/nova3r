...
def sample_sdf(mesh, random_surface, sharp_surface):
    ...
    # 1. Volume Points
    try:
        vol_sdf, I, C, N = igl.signed_distance(
            vol_points.astype(np.float64), V_np, F_np, sign_type=sign_type
        )
    except Exception:
        vol_sdf, I, C, N = igl.signed_distance(
            vol_points.astype(np.float64), V_np, F_np
        )

    # 2. Random Near Points
    try:
        random_near_sdf, I, C, N = igl.signed_distance(
            random_near_points.astype(np.float64), V_np, F_np, sign_type=sign_type
        )
    except Exception:
        random_near_sdf, I, C, N = igl.signed_distance(
            random_near_points.astype(np.float64), V_np, F_np
        )

    # 3. Sharp Near Points
    try:
        sharp_near_sdf, I, C, N = igl.signed_distance(
            sharp_near_points.astype(np.float64), V_np, F_np, sign_type=sign_type
        )
    except Exception:
        sharp_near_sdf, I, C, N = igl.signed_distance(
            sharp_near_points.astype(np.float64), V_np, F_np
        )
    vol_label = -vol_sdf
...
def convert_glb_to_ply(input_glb_path, output_ply_path=None):
    """
    Converts a .glb (or .gltf) file to a standard .ply mesh file.
    If output_ply_path is not specified, it replaces the extension of the input file.
    """
    if not output_ply_path:
        output_ply_path = os.path.splitext(input_glb_path)[0] + '.ply'
        
    print(f"🔄 Converting {input_glb_path} to {output_ply_path}...")
    
    # Load the GLB file (force='mesh' consolidates scenes/nodes into a single mesh)
    mesh = trimesh.load(input_glb_path, force='mesh')
    
    # Export explicitly to PLY format
...
if __name__ == '__main__':
    ...
    mc_verts, mc_faces = Watertight(V, F, epsilon=2.0/512, grid_res=256)
