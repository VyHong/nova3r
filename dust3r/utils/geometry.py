...
def to_plucker(pose, intrinsics, img_shape):
    """Convert pose and intrinsics to plucker rays

    pose: camera 2 to camera 1
    """
    # create a pixel grid
    B = pose.shape[0]
    # TODO: assume same shape
    H, W = img_shape
    y, x = torch.meshgrid(torch.arange(H), torch.arange(W))
    # concat 
    grid = torch.stack((x, y), dim=-1).view(1, H, W, 2).expand(B, -1, -1, -1).clone()

    grid = rearrange(grid, 'b h w c -> b (h w) c')
    grid = torch.cat((grid, torch.ones_like(grid[:, :, :1])), dim=-1).to(pose.device).float()
    # d = R^T @ K^-1 @ [x, y, 1]

    K_inv = intrinsics.inverse()
    Rot = pose[:, :3, :3]
    tvec = pose[:, :3, 3]
    ray_origins = tvec[:, None, :].expand(B, H*W, 3)
    x = torch.einsum('bij, bnj->bni', K_inv, grid)
    ray_directions = torch.einsum('bij, bnj->bni', Rot, x)
    ray_directions = ray_directions / ray_directions.norm(dim=-1, keepdim=True)

    plucker_normal = torch.cross(ray_directions, ray_origins, dim=-1)
    plucker_normal = rearrange(plucker_normal, 'b (h w) c -> b c h w', h=H, w=W)      
    ray_directions = rearrange(ray_directions, 'b (h w) c -> b c h w', h=H, w=W)
    rays = torch.cat((ray_directions, plucker_normal), dim=1)
...
def filter_depth(depth_np, threshold=0.1):
    ...
    if len(depth_np.shape) == 2:
        depth_np_new = depth_np_new.squeeze(-1)
...
def filter_pts3d_with_intrinsics(pts3d, intrinsics, camera_pose, image_shape):
        """
        Filter 3D points to keep only those that project within the image bounds.
        
        Args:
            pts3d: (N, 3) array of 3D points in world coordinates
            intrinsics: (3, 3) camera intrinsics matrix
            camera_pose: (4, 4) camera-to-world transformation matrix
            image_shape: (H, W) shape of the image
            
        Returns:
            filtered_pts3d: (M, 3) array of filtered 3D points
        """
        if pts3d.shape[0] == 0:
            return pts3d
            
        # Convert world-to-camera transformation (inverse of camera_pose)
        ...
        pts3d_camera = (world_to_camera @ pts3d_homogeneous.T).T[:, :3]
        
        # Filter points behind the camera (negative Z)
        valid_depth = pts3d_camera[:, 2] > 0
        
        # Project points to image plane
        fx, fy = intrinsics[0, 0], intrinsics[1, 1]
        cx, cy = intrinsics[0, 2], intrinsics[1, 2]
        
        x_proj = pts3d_camera[:, 0] / pts3d_camera[:, 2] * fx + cx
        y_proj = pts3d_camera[:, 1] / pts3d_camera[:, 2] * fy + cy
        ...
        # Filter and return valid points
        filtered_pts3d = pts3d[valid_mask]
        
        # print(f'After filtering - pts3d.shape: {filtered_pts3d.shape} (kept {np.sum(valid_mask)}/{len(pts3d)} points)')
...
def scale_shift_global_optimization(pts3d_pred, pts3d_gt, mask=None):
    """
    Global optimization approach using Chamfer distance
    """
    def chamfer_distance(params):
        scale, shift_x, shift_y, shift_z = params
        shift = np.array([shift_x, shift_y, shift_z])
        aligned = scale * pts3d_pred + shift
        
        # Bidirectional Chamfer distance
        dist1 = np.min(cdist(aligned, pts3d_gt), axis=1).mean()
        dist2 = np.min(cdist(pts3d_gt, aligned), axis=1).mean()
        return dist1 + dist2
    
    # Initial guess
    initial_params = [1.0, 0.0, 0.0, 0.0]  # scale=1, shift=0
    
    result = minimize(chamfer_distance, initial_params, method='BFGS')
    scale = result.x[0]
    shift = result.x[1:4]
...
def blender_to_opencv_pose(M, from_mapping="world_to_camera"):
    """
    Convert a Blender camera matrix to OpenCV 4x4 poses.

    Args:
        M (ndarray): Blender matrix, shape (3,4) or (4,4).
            - If from_mapping == "world_to_camera": M maps X_w -> X_c^Blender (extrinsic).
            - If from_mapping == "camera_to_world": M maps X_c^Blender -> X_w (pose).
        from_mapping (str): "world_to_camera" or "camera_to_world".

    Returns:
        T_cw_cv (ndarray): 4x4 OpenCV world->camera matrix (extrinsic).
        T_wc_cv (ndarray): 4x4 OpenCV camera->world matrix (pose).
    """
    _R_BCAM2CV = np.diag([1.0, -1.0, -1.0])
    
    M = np.asarray(M, dtype=float)
    if M.shape not in [(3,4), (4,4)]:
        raise ValueError("M must be 3x4 or 4x4")

    # Extract R, t
    R_B = M[:3, :3]
    t_B = M[:3, 3]

    if from_mapping == "world_to_camera":
        # Blender (w->c) -> OpenCV (w->c): apply camera-frame flip
        R_cv = _R_BCAM2CV @ R_B
        t_cv = _R_BCAM2CV @ t_B
        ...
        T_wc_cv = np.eye(4)
        T_wc_cv[:3, :3] = R_cv.T
        T_wc_cv[:3,  3] = -R_cv.T @ t_cv

    elif from_mapping == "camera_to_world":
        # Blender (c->w) pose -> OpenCV (c->w): right-multiply by flip in the camera block
        T_wc_B = np.eye(4)
        T_wc_B[:3, :3] = R_B
        T_wc_B[:3,  3] = t_B
        ...
        T_wc_cv = T_wc_B @ S
        T_cw_cv = np.linalg.inv(T_wc_cv)

    else:
        raise ValueError("from_mapping must be 'world_to_camera' or 'camera_to_world'")

    return T_cw_cv, T_wc_cv