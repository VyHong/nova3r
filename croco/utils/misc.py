...
def init_distributed_mode(args):
    ...
    torch.distributed.init_process_group(backend=args.dist_backend, init_method=args.dist_url,
                                         world_size=args.world_size, rank=args.rank, timeout=datetime.timedelta(seconds=1800))  # 30 minutes
    ...
    def __call__(self, loss, optimizer, clip_grad=None, parameters=None, create_graph=False, update_grad=True):
        ...
        if update_grad:
            ...
            if clip_grad is not None:
                ...
                if norm > clip_grad * 2.0:
                    print(f"Warning: [{update_grad}] the total norm of the gradients ({norm:.2f}) is larger than the clipping threshold ({clip_grad}), so the gradients are being clipped. .")

                if norm > clip_grad * 10:
                    skip_optimizer_step = True
                    print(f"Warning: [{update_grad}] the total norm of the gradients ({norm:.2f}) is much larger (10x) than the clipping threshold ({clip_grad}), so the optimizer step is being skipped to avoid NaN values. .")
...
def load_model(args, model_without_ddp, optimizer, loss_scaler):
    ...
    if args.resume is not None:
        ...
        # DEBUG: Check for NaN in loaded model weights
        print("[DEBUG] Checking loaded model weights for NaN...")
        nan_params = []
        for name, param in model_without_ddp.named_parameters():
            if torch.isnan(param).any():
                nan_count = torch.isnan(param).sum().item()
                nan_params.append((name, nan_count, param.numel()))
                print(f"  [ERROR] NaN in parameter: {name}, count={nan_count}/{param.numel()}")
        
        if nan_params:
            print(f"[ERROR] Found NaN in {len(nan_params)} parameters after loading checkpoint!")
            print("  This indicates the checkpoint itself contains NaN values.")
        else:
            print("[DEBUG] No NaN found in loaded model weights.")
...
def get_parameter_groups(model, weight_decay, layer_decay=1.0, skip_list=(), no_lr_scale_list=[], exclude_layers=[]):
    ...
    for name, param in model.named_parameters():
        ...
        if any(name.startswith(exclude_prefix) for exclude_prefix in exclude_layers):
            continue
