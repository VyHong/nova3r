...
from models.blocks import Block, DecoderBlock, PatchEmbed, TimeEmbedBlock, TimeEmbedDecoderBlock
...
class CroCoNet(nn.Module):
    ...
    def __init__(self,
                 ...
                 emb_channels=None,      # number of channels for the positional embedding
                 time_embed_mode=None,       # whether to add a time embedding
                ):
        ...
        if self.time_embed_mode is not None:
            use_adaLN = False
            if time_embed_mode == 'adaLN':
                use_adaLN = True

            print('Using time embedding in the encoder', self.time_embed_mode)
            self.enc_blocks = nn.ModuleList([
                TimeEmbedBlock(enc_embed_dim, enc_num_heads, emb_channels=emb_channels, use_adaLN=use_adaLN, mlp_ratio=mlp_ratio, qkv_bias=True, norm_layer=norm_layer, rope=self.rope)
    ...
    def _set_decoder(self, enc_embed_dim, dec_embed_dim, dec_num_heads, dec_depth, mlp_ratio, norm_layer, norm_im2_in_dec):
        ...
        if self.time_embed_mode is not None:
            use_adaLN = False
            if self.time_embed_mode == 'adaLN':
                use_adaLN = True
            print('Using time embedding in the decoder', self.time_embed_mode)
            self.dec_blocks = nn.ModuleList([
            TimeEmbedDecoderBlock(dec_embed_dim, dec_num_heads, emb_channels=self.emb_channels, use_adaLN=use_adaLN, mlp_ratio=mlp_ratio, qkv_bias=True, norm_layer=norm_layer, norm_mem=norm_im2_in_dec, rope=self.rope)
